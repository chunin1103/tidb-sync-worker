/**
 * Spiral Animation - Vanilla JS Port
 * Original: React/TypeScript component from 21st.dev
 * Ported for Agent Garden splash screen
 */
...
class AnimationController {
    ...
    // Setup seeded random generator for consistent star positions
    ...
    // Rotation transformation with bounce effect
...
class Star {
    constructor(cameraZ, cameraTravelDistance) {
        ...
        // Store for access in render
        this.cameraZ = cameraZ;
        this.cameraTravelDistance = cameraTravelDistance;
    }
    ...
    render(p, controller) {
        ...
        if (q > 0) {
            ...
            // Mixed easing for natural animation
            ...
            if (displacementProgress < 0.3) {
                ...
                // Final phase: spiral expansion (30%)
...
class SplashScreen {
    constructor(options = {}) {
        this.containerId = options.containerId || 'splash-screen';
        this.canvasId = options.canvasId || 'spiral-canvas';
        this.onComplete = options.onComplete || (() => {});
        this.autoHideDelay = options.autoHideDelay || null; // null = click to enter
    ...
    setupCanvas() {
        ...
        this.controller = new AnimationController(this.canvas, ctx, dpr, size);
    }
    ...
    setupEventListeners() {
        ...
        // Click to enter
        const enterButton = document.getElementById('splash-enter-btn');
        if (enterButton) {
            enterButton.addEventListener('click', () => this.hide());
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('splash-screen')) {
        window.splashScreen = new SplashScreen({
            containerId: 'splash-screen',
            canvasId: 'spiral-canvas',
            onComplete: () => {
